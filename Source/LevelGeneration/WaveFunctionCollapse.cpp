...
void AWaveFunctionCollapse::CreateInternalTileSet()
{
	TileSetInternal.Empty();
	for (FTile Tile : TileSet)
	{
		TileSetInternal.Add(Tile);
		if (Tile.bCreateRotatedInstances)
		{
			for (size_t i = 1; i < 4; i++)
			{
				FTile NewTile = Tile;
				NewTile.MeshYawRotation = 90 * i;
				NewTile.SideTypes[Side::FRONT] = Tile.SideTypes[Side(((uint8)Side::FRONT + i) % 4)];
				NewTile.SideTypes[Side::RIGHT] = Tile.SideTypes[Side(((uint8)Side::RIGHT + i) % 4)];
				NewTile.SideTypes[Side::BACK] = Tile.SideTypes[Side(((uint8)Side::BACK + i) % 4)];
				NewTile.SideTypes[Side::LEFT] = Tile.SideTypes[Side(((uint8)Side::LEFT + i) % 4)];
				TileSetInternal.Add(NewTile);
			}
		}
	}
}
	...
	for (size_t i = 0; i < Rows; i++)
	{
		for (size_t j = 0; j < Columns; j++)
		{
			if (possibleOptions[i][j].Num() == 0)
			{
				...
				possibleOptions[i][j] = TileSetInternal.FilterByPredicate([Front, Back, Right, Left](const FTile& tile)
					{
...
void AWaveFunctionCollapse::PlaceMeshes(TArray<TArray<TArray<FTile>>>& possibleOptions)
{
	for (size_t i = 0; i < possibleOptions.Num(); i++)
	{
		for (size_t j = 0; j < possibleOptions[i].Num(); j++)
		{
			...
			auto actor = GetWorld()->SpawnActor<AStaticMeshActor>(tileCenter, FRotator{ 0, possibleOptions[i][j][0].MeshYawRotation, 0}, FActorSpawnParameters{});
			actor->SetActorRelativeScale3D(FVector(MeshScale));
...
void AWaveFunctionCollapse::AdjustNavMeshVolume()
{
	...
	NavMeshBounds->SetActorScale3D(FVector(Rows * TileSize, Columns * TileSize, 1000.f));
}
